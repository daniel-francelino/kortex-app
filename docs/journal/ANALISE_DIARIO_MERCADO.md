...
| **Criptografia ponta-a-ponta** | ✅ Implementado (2026-08-24) | Título e conteúdo cifrados no cliente (AES-256-GCM), servidor nunca vê texto claro. Com uma diferença deliberada dos concorrentes pesquisados: uma terceira cópia da chave de dados é cifrada com uma chave pública de recuperação do Kortex, permitindo suporte assistido se a pessoa perder a senha e o código de recuperação — abrindo mão de "zero-knowledge" estrito em troca disso. Ver `1.JOURNAL.md`, seção 14, e `PLANO_CRIPTOGRAFIA_PONTA_A_PONTA.md`. |
...
13. ✅ **Notas semanais/mensais (rollup periódico)** — estilo Periodic Notes do Obsidian: uma nota de resumo da semana/mês que referencia ou resume as entradas diárias do período. Implementado em 2026-08-25: quinta visão "Períodos", nota de texto livre por semana/mês (não auto-resumida por IA) + lista das entradas diárias do período. Ver `1.JOURNAL.md`, seção 15.1.
...
15. ✅ **Exportação (Markdown/PDF/backup)** — tabela-stakes em qualquer app de diário sério, hoje ausente tanto aqui quanto em Notas. Implementado em 2026-08-25 para os dois módulos: Markdown (.md) e PDF (via impressão nativa do navegador) por entrada/nota, mais "Exportar tudo" no Diário (Markdown combinado ou backup JSON, decifrando primeiro se E2EE estiver ligado). Ver `1.JOURNAL.md`, seção 15.2.
16. ✅ **Criptografia ponta-a-ponta** — maior esforço da lista (muda o modelo de armazenamento inteiro), mas é exatamente o que a pesquisa de mercado aponta como diferencial de confiança nos apps mais respeitados da categoria (seção 6.3). Implementado em 2026-08-24 com um requisito de produto que os concorrentes pesquisados não têm: recuperação assistida por suporte via uma terceira cópia de chave, não só "senha + código pessoal". Ver `1.JOURNAL.md`, seção 14.
17. ✅ **Geotag/clima automático na entrada** — bom-ter, baixo diferencial percebido comparado aos itens acima; only fazer se sobrar prioridade. Implementado em 2026-08-26, versão enxuta: captura automática ao abrir a página (pede permissão de geolocalização sozinho; se negada, não mostra nada) + consulta a Open-Meteo (gratuita, sem chave), badge de temperatura/condição. Sem reverse geocoding (nome de cidade). Ver `1.JOURNAL.md`, seção 15.3.
