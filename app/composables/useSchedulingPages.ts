...
function normalizeSchedulingPage(input: unknown): SchedulingPage {
  ...
  return {
    ...
    durationMinutes: Number(
      page.durationMinutes ?? page.duration_minutes ?? 30,
    ),
    locationType: (page.locationType ??
      page.location_type ??
      "video_link") as SchedulingPage["locationType"],
    locationDetails:
      (page.locationDetails as string | null) ??
      (page.location_details as string | null) ??
      null,
    timezone: String(page.timezone ?? ""),
    color: (page.color as string | null) ?? null,
    coverImageUrl:
      (page.coverImageUrl as string | null) ??
      (page.cover_image_url as string | null) ??
      null,
    slug: String(page.slug ?? ""),
    showOnProfile: Boolean(page.showOnProfile ?? page.show_on_profile ?? true),
    bufferBeforeMinutes: Number(
      page.bufferBeforeMinutes ?? page.buffer_before_minutes ?? 0,
    ),
    bufferAfterMinutes: Number(
      page.bufferAfterMinutes ?? page.buffer_after_minutes ?? 0,
    ),
    slotIncrementMinutes: Number(
      page.slotIncrementMinutes ?? page.slot_increment_minutes ?? 15,
    ),
    ...
    maxBookingsPerDay:
      (page.maxBookingsPerDay as number | null) ??
      (page.max_bookings_per_day as number | null) ??
      null,
    calendarEventTitleTemplate:
      (page.calendarEventTitleTemplate as string | null) ??
      (page.calendar_event_title_template as string | null) ??
      null,
    cancellationEnabled: Boolean(
      page.cancellationEnabled ?? page.cancellation_enabled ?? true,
    ),
    rescheduleEnabled: Boolean(
      page.rescheduleEnabled ?? page.reschedule_enabled ?? true,
    ),
    cancellationMinNoticeHours:
      (page.cancellationMinNoticeHours as number | null) ??
      (page.cancellation_min_notice_hours as number | null) ??
      null,
    cancellationReasonRequired: Boolean(
      page.cancellationReasonRequired ?? page.cancellation_reason_required,
    ),
    hideDetailsOnManagePage: Boolean(
      page.hideDetailsOnManagePage ?? page.hide_details_on_manage_page,
    ),
    requiresConfirmation: Boolean(
      page.requiresConfirmation ?? page.requires_confirmation,
    ),
    ...
    archivedAt:
      (page.archivedAt as string | null) ??
      (page.archived_at as string | null) ??
      null,
    availabilityRules: Array.isArray(page.availabilityRules)
      ? page.availabilityRules.map(normalizeAvailabilityRule)
      : undefined,
    questions: Array.isArray(page.questions)
      ? page.questions.map(normalizeQuestion)
      : undefined,
    bookingsCount:
      typeof page.bookingsCount === "number" ? page.bookingsCount : undefined,
  };
}
...
function normalizeBooking(input: unknown): Booking {
  ...
  return {
    ...
    cancellationReason:
      (b.cancellationReason as string | null) ??
      (b.cancellation_reason as string | null) ??
      null,
    createdAt: String(b.createdAt ?? b.created_at ?? ""),
    updatedAt: String(b.updatedAt ?? b.updated_at ?? ""),
    cancelledAt:
      (b.cancelledAt as string | null) ??
      (b.cancelled_at as string | null) ??
      null,
    startAt:
...
function buildOptimisticQuestions(
...
export function useSchedulingPages() {
  ...
  const pages = computed(() =>
    pageIds.value
      ...
      .filter((p): p is SchedulingPage => !!p),
  );
  ...
  async function createSchedulingPage(
    payload: CreateSchedulingPagePayload,
    ...
    const result = await runOptimisticAction<SchedulingPage>({
      ...
      request: () =>
        $fetch("/api/appointments/scheduling-pages", {
          method: "POST",
          body: { ...payload, id },
  ...
  async function fetchSchedulingPage(
    id: string,
  ): Promise<SchedulingPage | null> {
    try {
  ...
  async function updateSchedulingPage(
    id: string,
    payload: UpdateSchedulingPagePayload,
  ): Promise<SchedulingPage | null> {
    ...
    const optimisticPage: SchedulingPage = {
      ...
      calendarId:
        payload.calendarId !== undefined
          ...
          : (previous?.calendarId ?? ""),
      title:
        payload.title !== undefined ? payload.title : (previous?.title ?? ""),
      description:
      ...
      durationMinutes:
        payload.durationMinutes !== undefined
          ...
          : (previous?.durationMinutes ?? 30),
      locationType:
        payload.locationType !== undefined
          ? payload.locationType
          : (previous?.locationType ?? SchedulingLocationType.VideoLink),
      locationDetails:
        payload.locationDetails !== undefined
          ? (payload.locationDetails ?? null)
          : (previous?.locationDetails ?? null),
      timezone:
        payload.timezone !== undefined
          ? payload.timezone
          : (previous?.timezone ?? ""),
      color:
        payload.color !== undefined ? payload.color : (previous?.color ?? null),
      coverImageUrl:
      ...
      showOnProfile:
        payload.showOnProfile !== undefined
          ? payload.showOnProfile
          : (previous?.showOnProfile ?? true),
      bufferBeforeMinutes:
        payload.bufferBeforeMinutes !== undefined
          ? payload.bufferBeforeMinutes
          : (previous?.bufferBeforeMinutes ?? 0),
      bufferAfterMinutes:
        payload.bufferAfterMinutes !== undefined
          ? payload.bufferAfterMinutes
          : (previous?.bufferAfterMinutes ?? 0),
      slotIncrementMinutes:
        payload.slotIncrementMinutes !== undefined
          ? payload.slotIncrementMinutes
          : (previous?.slotIncrementMinutes ?? 15),
      minNoticeHours:
        payload.minNoticeHours !== undefined
          ? payload.minNoticeHours
          : (previous?.minNoticeHours ?? 4),
      maxAdvanceDays:
        payload.maxAdvanceDays !== undefined
          ? payload.maxAdvanceDays
          : (previous?.maxAdvanceDays ?? 60),
      maxBookingsPerDay:
        payload.maxBookingsPerDay !== undefined
          ? payload.maxBookingsPerDay
          : (previous?.maxBookingsPerDay ?? null),
      calendarEventTitleTemplate:
        payload.calendarEventTitleTemplate !== undefined
          ? payload.calendarEventTitleTemplate
          : (previous?.calendarEventTitleTemplate ?? null),
      cancellationEnabled:
        payload.cancellationEnabled !== undefined
          ? payload.cancellationEnabled
          : (previous?.cancellationEnabled ?? true),
      rescheduleEnabled:
        payload.rescheduleEnabled !== undefined
          ? payload.rescheduleEnabled
          : (previous?.rescheduleEnabled ?? true),
      cancellationMinNoticeHours:
        payload.cancellationMinNoticeHours !== undefined
          ? payload.cancellationMinNoticeHours
          : (previous?.cancellationMinNoticeHours ?? null),
      cancellationReasonRequired:
        payload.cancellationReasonRequired !== undefined
          ? payload.cancellationReasonRequired
          : (previous?.cancellationReasonRequired ?? false),
      hideDetailsOnManagePage:
        payload.hideDetailsOnManagePage !== undefined
          ? payload.hideDetailsOnManagePage
          : (previous?.hideDetailsOnManagePage ?? false),
      requiresConfirmation:
        payload.requiresConfirmation !== undefined
          ? payload.requiresConfirmation
          : (previous?.requiresConfirmation ?? false),
      ...
      availabilityRules: payload.availabilityRules
        ? buildOptimisticRules(payload.availabilityRules)
        : previous?.availabilityRules,
      questions: payload.questions
        ? buildOptimisticQuestions(payload.questions)
        : previous?.questions,
    ...
    const result = await runOptimisticAction<SchedulingPage>({
      ...
      rollback: () => {
        if (previous) upsertPageInStore(previous);
      },
      request: () =>
        $fetch(`/api/appointments/scheduling-pages/${id}`, {
          method: "PATCH",
          body: payload,
        }).then(normalizeSchedulingPage),
      reconcile: (serverPage) => upsertPageInStore(serverPage),
  ...
  async function archiveSchedulingPage(id: string): Promise<boolean> {
    ...
    const result = await runOptimisticAction<{ success: true }>({
      apply: () => {
        if (previous)
          upsertPageInStore({ ...previous, isActive: false, archivedAt: now });
        pageIds.value = pageIds.value.filter((i) => i !== id);
      },
      rollback: () => {
        if (previous) {
          upsertPageInStore(previous);
          if (!pageIds.value.includes(id))
            pageIds.value = [...pageIds.value, id];
        }
      },
      request: () =>
        $fetch(`/api/appointments/scheduling-pages/${id}/archive`, {
          method: "POST",
  ...
  async function regenerateShareToken(
    ...
    try {
      const data = await $fetch(
        `/api/appointments/scheduling-pages/${id}/regenerate-token`,
        { method: "POST" },
      );
  ...
  async function duplicateSchedulingPage(
    pageId: string,
  ): Promise<SchedulingPage | null> {
  ...
  async function fetchBookings(pageId: string): Promise<Booking[]> {
    try {
      const data = await $fetch<unknown[]>(
        `/api/appointments/scheduling-pages/${pageId}/bookings`,
      );
  ...
  async function approveBooking(
    ...
    try {
      const data = await $fetch(
        `/api/appointments/scheduling-pages/${pageId}/bookings/${bookingId}/approve`,
        { method: "POST" },
      );
      ...
      toast.add({
        title: "Erro",
        description: message ?? "Não foi possível aprovar a reserva.",
        color: "error",
      });
      return null;
    }
  ...
  async function cancelBookingAsHost(
    pageId: string,
    bookingId: string,
    reason?: string,
  ): Promise<Booking | null> {
    try {
      const data = await $fetch(
        `/api/appointments/scheduling-pages/${pageId}/bookings/${bookingId}/cancel`,
        {
          method: "POST",
          body: { reason },
        },
      ...
      toast.add({
        title: "Erro",
        description: message ?? "Não foi possível cancelar a reserva.",
        color: "error",
      });
      return null;
    }
  ...
  const {
    pendingMutations,
    pendingCount,
    dequeue: dequeueMutation,
    markRetry,
    ensureLoaded: ensureQueueLoaded,
  } = useMutationQueue();
  ...
  async function drainMutationQueue(): Promise<void> {
    ...
    const relevant = pendingMutations.value.filter((m) =>
      (SCHEDULING_ENTITIES as readonly string[]).includes(m.entity),
    );
    ...
    try {
      ...
      for (const mutation of queue) {
        ...
        try {
          ...
          if (!isOnline.value) break;
          console.error(
            "[offline-sync] scheduling-page mutation failed",
            mutation,
            err,
          );
      ...
      if (replayedAny) {
        ...
        toast.add({
          title: "Sincronizado",
          description:
            "Suas alterações offline nas páginas de agendamento foram salvas.",
          color: "success",
        });
      }
    }
