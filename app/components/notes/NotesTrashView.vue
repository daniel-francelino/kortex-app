...
function confirmPermanentDelete() {
  ...
  if (confirmTargets.value.length === 1)
    emit("permanent-delete", confirmTargets.value[0]!);
  ...
  <div class="flex flex-col h-full min-h-0">
    <div class="px-4 sm:px-6 py-4 border-b border-default shrink-0">
      <div class="flex items-center justify-between gap-2">
        <h2
          class="text-base font-semibold text-highlighted flex items-center gap-2"
        >
      ...
      <p class="text-xs text-muted mt-0.5">
        Notas e pastas excluídas ficam aqui até serem restauradas ou excluídas
        permanentemente — itens com mais de 30 dias na lixeira são removidos
        automaticamente.
      </p>
    </div>
      ...
      <template #header>
        <h3 class="text-lg font-semibold">
          <template v-if="confirmTargets && confirmTargets.length === 1">
            Excluir "{{ confirmTargets[0]!.title || "Sem título" }}"
            permanentemente?
          </template>
          <template v-else>
      ...
      <template #body>
        <p class="text-sm text-muted">
          <template v-if="confirmTargets && confirmTargets.length === 1">
            ...
            <template v-else> Essa nota será removida para sempre. </template>
          </template>
          <template v-else>
            Isso remove {{ confirmTargets?.length ?? 0 }} itens (e o conteúdo de
            pastas incluídas) para sempre.
          </template>
